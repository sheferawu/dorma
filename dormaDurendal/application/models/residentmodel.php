...
class Residentmodel extends CI_Model {
   ...
   function searchViolator($fname,$mname,$lname){
   	
   	$this->{$this->db_group_name2} = $this->load->database($this->db_group_name2, TRUE);
    $query = "Select FirstName,MidName,LastName From resident where FirstName Like '$fname%' AND MidName Like '$mname%' AND LastName Like '$lname%'";
   	$table="<table id=\"sg\" border=\"0\">";
    $res=$this->{$this->db_group_name2}->query($query);
   	if($res->num_rows()>0){
   		$cnt = 0;
   		foreach($res->result() as $r){
   			...
   			$r->MidName = ucwords(strtolower($r->MidName));
   			$r->LastName= ucwords(strtolower($r->LastName));
   			$val ="'".$r->FirstName."@".$r->MidName ."@".$r->LastName."'";
   			$table.="<tr><td onclick = \"setValue3($val)\"  onkeydown = \"setValue3($val)\" >Click: $r->FirstName $r->MidName  $r->LastName</td></tr>";
   			$cnt++;
   			if($cnt>=1)break;
   		}
   	}else{
...
$cnt = 0;
		...
		foreach ($query->result() as $row)
		{
   		...
   		$query = $this->{$this->db_group_name2}->query("SELECT * from violation WHERE FirstName='$fname' AND LastName='$lname' AND MidName='$mname'");
		$arrResField = $this->Optionmodel->returnFieldsArray("violation");
   ...
   function delete_resident($fname,$lname,$mname){
		...
		$this->{$this->db_group_name2}->query("DELETE FROM org WHERE FirstName= '$fname' AND LastName= '$lname' AND MidName= '$mname'");
		$this->{$this->db_group_name2}->query("DELETE FROM othersourcesincome WHERE FirstName= '$fname' AND LastName= '$lname' AND MidName= '$mname'");
		$this->{$this->db_group_name2}->query("DELETE FROM balance WHERE FirstName= '$fname' AND LastName= '$lname' AND MidName= '$mname'");
		$this->{$this->db_group_name2}->query("DELETE FROM appliances WHERE FirstName= '$fname' AND LastName= '$lname' AND MidName= '$mname'");
   ...
   function view_entryT($arrcol,$fname,$lname,$mname){//view transient
   		$this->{$this->db_group_name2} = $this->load->database($this->db_group_name2, TRUE);
   ...
   function view_entry($table,$arrcol,$fname,$lname,$mname){//view student
		$this->{$this->db_group_name2} = $this->load->database($this->db_group_name2, TRUE);
   ...
   function editT($arrTrans,$_GET){//edit  transient
   	$fname =$_GET["fname"];
   	$mname =$_GET["mname"];
   	$lname =$_GET["lname"];
   ...
   function add_transientEntry($_POST){//add transient
		$str="";
		...
		$sql = "Select LastName from transient where FirstName ='$_POST[firstname]'  &&MidName ='$_POST[middlename]' &&LastName ='$_POST[lastname]'";
		...
		$temp ="";
  		...
  		foreach($result->result() as $row){
			$temp =  "Transient not added!<br/>$_POST[firstname] $_POST[middlename] $_POST[lastname] is already on the record!";
			$_SESSION["trans"] = $temp;
			return;
  		}
   ...
   function add_entry($_POST){//add student
...
if($age<=0) {
		...
		$temp =  "Resident not added!<br/>$fname $mname $lname's age is less than or equal to zero!";
...
$str.= $this->Optionmodel->viewTable("hobbies",$arrcol,$_POST["firstname"],$_POST["lastname"],$_POST["middlename"]);
	...
	if(isset($_POST["Talents0"])&&trim($_POST["Talents0"])!=""){
	...
	$sql = "Insert into talent
		...
		'$_POST[Talents0]' )";
$this->{$this->db_group_name2}->query($sql) ;
$arrcol = $this->Optionmodel->returnFieldsArray("talent");
$str.= $this->Optionmodel->viewTable("talent",$arrcol,$_POST["firstname"],$_POST["lastname"],$_POST["middlename"]);
...
if((isset($_POST["cg1name"])&&$_POST["cg1name"]!="")||(isset($_POST["cg2name"])&&$_POST["cg2name"]!="")||(isset($_POST["cfname"])&&$_POST["cfname"]!="")||(isset($_POST["cmname"])&&$_POST["cmname"]!="")){
...
if(str_replace("/","",$checkOut)==""||strlen(str_replace("/","",$checkOut)=="")<=1){
	$checkOut ="";
}
...
return $str;		
   ...
   function getAppRecords($date){
					...
					$app = $this->{$this->db_group_name2}->get();
  					$cnt =0;
					foreach ($app->result() as $row)
					{	$date2 = new DateTime($row->DateInstalled);
   ...
   function viewViolation($arrcol,$fname,$lname,$mname){
		  $this->{$this->db_group_name2} = $this->load->database($this->db_group_name2, TRUE);
    	
		
		$query =  $this->{$this->db_group_name2}->query("SELECT * FROM violation	
			WHERE FirstName= '$fname' AND LastName= '$lname' AND MidName= '$mname'");
			
		$result = "<table class=\"violation_table\">";
  		...
  		{ 
			foreach($arrcol as $q){
				//if(trim($row[$q])!=""){
					...
					if ($q == "DateOfViolation") $title = "Date of Violation";
					else if ($q == "ActionTaken") $title = "Action Taken";
   ...
   function updateTransient($_POST,$fname,$mname,$lname){
	$arrayKeys = array_keys($_POST);
		...
		for($i=0;$i<count($arrayValues);$i++){
			...
			$_POST["$arrayKeys[$i]"]=strtoupper($val);
...
if($age<=0) {
		...
		$temp =  "Resident not updated!<br/>$fname $mname $lname's age is less than or equal to zero!";
		...
		$data = array(
			...
			'Address' => trim(strtoupper($_POST["HomeAddress"])),
	...
	if($numApp>0){
		...
		$_POST["dateInstalled".$jj] = $_POST["dateInstalledMonth".$jj]."/".$_POST["dateInstalledDay".$jj]."/".$_POST["dateInstalledYear".$jj];
		
		$dateInstalled = $_POST["dateInstalled".$jj];
		
		$cnum = $_POST["controlNum".$jj];
		...
		/* UPDATE VIOLATION */
		$dov = $_POST["dovMonth"]."/".$_POST["dovDay"]."/".$_POST["dovYear"];	
		$d = $_POST["vMonth"]."/".$_POST["vDay"]."/".$_POST["vYear"];
		...
		$data = array(
			...
			'LastName' => strtoupper($_POST["lastname"]),
			'DateOfViolation' => $dov,	
			'ActionTaken' => strtoupper($_POST["actionTaken"]),
			'Date' => $d,
			'Nature' => strtoupper($_POST["nature"]),
       	);
   ...
   function getNumberOutIn($month){//get login date and logout date og resident
   ...
   function add_violation($_POST){
		...
		for($i=0;$i<count($arrayValues);$i++){
 		...
 		$action = trim(strtoupper($_POST["actionTaken"]));
 		$date2 = $_POST["MonthAction"]."/".$_POST["DayAction"]."/".$_POST["YearAction"];
 		if ($date2 == "0//") $date2 = "";
 		$nature = trim(strtoupper($_POST["violationNature"]));
		 	
 		$sql = "Insert into violation values
 			('$lname',
 			'$mname',
 			'$fname',
 			'$date1',
   ...
   function getViolationList(){
 	...
 	$result = " <form name=\"deleteViolationForm\" method=\"POST\" action=\"index.php?c=resident&m=removeViolation\">
				<table border>
				<tr><th></th>
					<th>Name</th>
 	...
 	foreach($query->result() as $row){
			...
			$id = $row->FirstName."@".$row->MidName."@".$row->LastName."@".$row->DateOfViolation."@".$row->Nature."@".$row->ActionTaken."@".$row->Date;
 			$result.="<tr>";
			$result.="<td><input type=\"radio\" name=\"deleteViolation\" value=\"$id\"/></td>";
			$result.="<td>".ucwords(strtolower($name))."</td>";
			$result.="<td>".ucwords(strtolower($row->DateOfViolation))."</td>";
			$result.="<td>".ucwords(strtolower($row->Nature))."</td>";
			$result.="<td>".ucwords(strtolower($row->ActionTaken))."</td>";	
			$result.="<td>".ucwords(strtolower($row->Date))."</td>";	
			$result.="</tr>";	
	}
	...
	$result.="</table><br/>
				<input type=\"Submit\" Value=\"Delete\" name=\"delViolation\"/>
			</form>";
   ...
   function remove_violation($name){
 		$nameArray = explode("@", $name);
 	
 		$this->{$this->db_group_name2} = $this->load->database($this->db_group_name2, TRUE);
        $this->{$this->db_group_name2}->query("DELETE FROM violation WHERE FirstName= '$nameArray[0]' AND MidName= '$nameArray[1]' AND LastName= '$nameArray[2]' 
        AND DateOfViolation= '$nameArray[3]'AND Nature= '$nameArray[4]'AND ActionTaken= '$nameArray[5]'AND Date= '$nameArray[6]' ");
 }
