...
function validateViolationForm(form){
	var proceed = true;
	
	with(form){
		
		if (firstname.value.trim().toUpperCase() != "" && firstname.value.trim().length <= 30)
			document.getElementById("fnameV").style.background="white";
		else{
			document.getElementById("fnameV").style.background="#FCBBBB";
			proceed = false;
		}
		
		if (lastname.value.trim().toUpperCase() != "" && lastname.value.trim().length <= 15)
			document.getElementById("lnameV").style.background="white";
		else{
			document.getElementById("lnameV").style.background="#FCBBBB";
			proceed = false;
		}
		
		if (middlename.value.trim().toUpperCase() != "" && middlename.value.trim().length <= 15)
			document.getElementById("mnameV").style.background="white";
		else{
			document.getElementById("mnameV").style.background="#FCBBBB";
			proceed = false;
		}
		
		if (violationNature.value.trim().toUpperCase() != "" && violationNature.value.trim().length <= 20)
			document.getElementById("violationNature").style.background="white";
		else{
			document.getElementById("violationNature").style.background="#FCBBBB";
			proceed = false;
		}
		...
		if (actionTaken.value.trim().length <= 30)
			document.getElementById("actionTaken").style.background="white";
		else{
			document.getElementById("actionTaken").style.background="#FCBBBB";
			proceed = false;
		}
	}
...
function validateWorkloadForm(projectForm){
 ...
 function validateTransForm(transientForm){
	...
	with(transientForm){
		if (ControlNum.value.trim() != "" && ControlNum.value.length <= 20 && ControlNum.value.length > 0)
			cno.innerHTML = "";
		else
		{
			cno.innerHTML = "Invalid control number";
			proceed = false;
		}
...
{	
	...
	with(residentForm)
	{
		...
		if (!proceed){
			...
			if (ln.innerHTML != "" || fn.innerHTML != "" || mn.innerHTML != "" || bday.innerHTML != "" || std.innerHTML != "" || ad.innerHTML != "")
				location = "#top";
			else if (MonthLI1.value == "M" || room.value == "" || OrNum.value == "" || MonthR.value == "" || Amount.value == "" || OrNumKey.value == "" || AmountKey.value == "" || MonthRec.value == "")
				location = "#bottom";
		}
...
var bool =0;
	...
	if(DayId[DayId.length-1]!='@'){
		document.getElementById(DayId).innerHTML='<select name="'+DayId+'"  >'+str+'</select>';
		}else{
		
		DayId2 = DayId.substr(0,DayId.length-1);

		document.getElementById(DayId).innerHTML='<select name="'+DayId2+'"  id="'+DayId2+'" >'+str+'</select>';
		
		}
	if(bool==0){
		var d = new Date();
		if(YearId[YearId.length-1]!='@'){
		
			document.getElementById(YearId).innerHTML='<select name="'+YearId+'"  >'+returnOpt2(d.getFullYear()+1,1950)+'</select>';
		}else{
			
			YearId2 = YearId.substr(0,YearId.length-1);

			document.getElementById(YearId).innerHTML='<select name="'+YearId2+'" id="'+YearId2+'" >'+returnOpt2(d.getFullYear()+1,1950)+'</select>';
		...
		document.getElementById(YearId).innerHTML='<select name="'+YearId+'" ><option value="">Year</select>';
				...
				YearId2 = YearId.substr(0,YearId.length-1);
				document.getElementById(YearId).innerHTML='<select name="'+YearId2+'" id="'+YearId2+'" ><option value="">Year</select>';
...
function copyValue(){
	document.getElementById("OrNumKey").value = document.getElementById("OrNum").value;
	document.getElementById("MonthRec").value = document.getElementById("MonthR").value;
	
	getMonth(document.getElementById("MonthRec").value,'DayRec','YearRec');
	
	document.getElementById("DayRec").value = document.getElementById("DayR").value;
	document.getElementById("YearRec").value = document.getElementById("YearR").value;
}
