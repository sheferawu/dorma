<html>
	...
	<body>
		...
		<form action="index.php?c=dorm&m=saveSettings" method="POST" name="dormForm"  onsubmit="return(validateDormForm(this))">
			<fieldset>
			<h3 class="dorm_title" onclick="dropDown('dorm_info')">Dormitory Settings</h3>
			<div class="dorm_info">
				<table class="addDorm">
					...
					<tr>
						<td>End Date:</td>
						<td>
							<select name="eMonth"  id="eMonth" onchange="getMonth(this.options[this.selectedIndex].value,'eDay@','eYear@')">
							<?php 
									$bool="";
									for($i=0;$i<12;$i++){
										$bool[$i] ="";
					...
					<tr class="d1">
						<td><input class="buttonSettings" type="button" value="Create Suggested Clusters" onclick="setCluster()"/><br/>Suggested Clusters<br/>Edit it if you want to: </td>
						<td><div = id="clusterDiv"><?php if(isset($dorm["Clusters"])){
							
							//echo $dorm["Clusters"];
							$clusters = explode("*",$dorm["Clusters"]);	
							$cnt = 0;
							foreach($clusters as $clus){
								echo "<input type = \"text\" value=\"$clus\" name=\"cluster$cnt\"/><br/>";
								$cnt++;
							}
			...
			<br/>
			<h3 class ="applianceSetting" onclick="dropDown('applianceSet')">Appliance Settings</h3>
			<div class="applianceSet">
			...
			<table class = "appButton">
				<tr>
					...
					<td colspan="3"><input class="buttonSettings2" type="button" value="Add Appliance" name="addApp" onclick="addAppSet('appSet')" /></td>
					...
					<br/><center><input class="buttonSettings2" type="submit" value="Submit" name="submitResident"/></center>
		</fieldset>
		</form>
		<fieldset>
		<h3 class="password_title" onclick="dropDown('password_info')">Change Password</h3>
		...
		<tr height="70px">
					<th colspan="3" style="text-align: left">						
						...
						Status: <div id="changepassword"></div>
					</th>
				</tr>
				
				<tr>
					<td>Current Password:</td>
					<td><input type="password" name="currPswd" id="currPswd" onblur="checkPassword(this.value)"  /></td>
					<td id="cp" class="error"><div id="checkPwd"></div></td>
				</tr>
					
				<tr>
					<td>New Password:</td>
					<td><input type="password" name="newPswd" id ="newPswd"/></td>
					<td id="np" class="error"></td>
				</tr>	
				
				<tr>
					<td colspan="3" id="pswdError"></td>
				</tr>
				<tr>
				<td><input class="buttonSettings2" type="button" value="Save Password" onclick="saveDormPass()"/></td>
				</tr>
				
		</table>
		</div>
		<br/>
		<h3 class="otherOptions_title" onclick="dropDown('otherOptions_info')">Other Options</h3>
		<div class="otherOptions_info">
		<table>
		<tr><td><input  class="buttonSettings" type="button" value = "New Term" onclick="loadXMLDoc('index.php?c=dorm&m=newterm')"/></td><td><div id="savePreV"></div></td></tr>
		<tr><td><input  class="buttonSettings" type="button" value = "Edit Previous Accounts" onclick="loadXMLDoc('index.php?c=dorm&m=editprev')"/></td><td><div id="savePreV"></div></td></tr>
		<tr><td><input  class="buttonSettings" type="button" value = "Update Balance" onclick="updateBal()"/></td><td><div id="updateBal"></div></td></tr>
